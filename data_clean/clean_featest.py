...
#for file_id in tqdm.tqdm(all_subjects):
    ...
    x = torch.load("/extern2/zgz/wq/sleep/sleep_stage_ppg/clean_mesa_300s/"+mode+"/raw/x_"+str(file_id)+".pt")
    ...
    ppg_peaks_l = nk.ppg_findpeaks(-1 * std_x, sampling_rate=256)
    ...
    RR_list = f_hr([i for i in range(0, std_x.shape[0], 256)])/256
    blood_list = f_bl([i for i in range(0, std_x.shape[0], 256)])/256
    ba_list = signal.resample(signal.filtfilt(b_l, a_l, std_x), int(std_x.shape[0]/256))
    am_list = f_h([i for i in range(0, std_x.shape[0], 256)]) - f_l([i for i in range(0, std_x.shape[0], 256)])
    ...
    for i in tqdm.tqdm(range(0, int(std_x.shape[0]/256-20), 10)):
        temp = hrvana.get_time_domain_features(RR_list[i:i+30] * 1000)
        ppg_rr = [temp['mean_nni'], temp['sdnn'], temp['sdsd']]
        temp = hrvana.get_frequency_domain_features(RR_list[i:i+30] * 1000)
        ppg_rr = ppg_rr + [temp['vlf'], temp['lf'], temp['hf'],
                        temp['lf_hf_ratio'], temp['total_power']]
        ppg_rrfea.append(ppg_rr)
        
        temp = hrvana.get_time_domain_features(am_list[i:i+30] * 1000)
        ppg_rr = [temp['mean_nni'], temp['sdnn'], temp['sdsd']]
        temp = hrvana.get_frequency_domain_features(am_list[i:i+30] * 1000)
        ppg_rr = ppg_rr + [temp['vlf'], temp['lf'], temp['hf'],
                        temp['lf_hf_ratio'], temp['total_power']]
        ppg_amfea.append(ppg_rr)

        temp = hrvana.get_time_domain_features(ba_list[i:i+30] * 1000)
        ppg_rr = [temp['mean_nni'], temp['sdnn'], temp['sdsd']]
        temp = hrvana.get_frequency_domain_features(ba_list[i:i+30] * 1000)
        ppg_rr = ppg_rr + [temp['vlf'], temp['lf'], temp['hf'],
                        temp['lf_hf_ratio'], temp['total_power']]
        ppg_bafea.append(ppg_rr)
        
        temp = hrvana.get_time_domain_features(blood_list[i:i+30] * 1000)
        ppg_rr = [temp['mean_nni'], temp['sdnn'], temp['sdsd']]
        temp = hrvana.get_frequency_domain_features(blood_list[i:i+30] * 1000)
        ppg_rr = ppg_rr + [temp['vlf'], temp['lf'], temp['hf'],
                        temp['lf_hf_ratio'], temp['total_power']]
    ...
    torch.save(torch.tensor(ppg_fea), '/extern2/zgz/wq/sleep/sleep_stage_ppg/clean_mesa_300s/'+mode+'/fea_32/x_'+file_id+'.pt')
