...
#for file_id in tqdm.tqdm(all_subjects):
    ...
    RR_list = f_hr([i for i in range(0, std_x.shape[0], hz / 2)])/hz
    blood_list = f_bl([i for i in range(0, std_x.shape[0], hz / 2)])/hz
    ba_list = signal.resample(signal.filtfilt(b_l, a_l, std_x), int(std_x.shape[0]/hz * 2))
    am_list = f_h([i for i in range(0, std_x.shape[0], hz / 2)]) - f_l([i for i in range(0, std_x.shape[0], hz / 2)])
    ...
    torch.save(torch.tensor(ppg_row), '/extern2/zgz/wq/sleep/sleep_stage_ppg/clean_mesa_300s/'+mode+'/ppg/x_'+file_id+'.pt')
    torch.save(torch.tensor(ppg_sig), '/extern2/zgz/wq/sleep/sleep_stage_ppg/clean_mesa_300s/'+mode+'/sig_4/x_'+file_id+'.pt')
