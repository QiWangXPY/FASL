...
'''
mode = mode + '_ahi'
with open(data_path + 'cfs/'+mode+'_label.pkl', 'rb') as f:
    test_label = pickle.load(f)
...
for i in tqdm(ref_all):
    ...
    sig_temp = adjust_length_2D(sig_temp, 600 * 128)
    sig_list = list()
    for j in range(0, sig_temp.size(1), 64):
        ...
        temp = adjust_length_2D(temp, 256)
        sig_list.append(temp.unsqueeze(1))
    sig[ref] = torch.concat(sig_list, axis = 1)
    ...
    for j in range(0, fea_temp.shape[1], 3):
        ...
        fea_temp_li.append( adjust_length_2D( fea_temp[:, beg : end], 11 ).reshape([-1]).unsqueeze(1) )
    fea_temp = torch.concat(fea_temp_li, axis = 1)
    fea_temp = adjust_length_2D(fea_temp, 1200)
    fea[ref] = fea_temp
...
'''
with open(data_path + 'mesa/10h_input/'+mode+'_ppg.pkl', 'wb') as f:
    pickle.dump(ppg, f)
with open(data_path + 'mesa/10h_input/'+mode+'_sig.pkl', 'wb') as f:
    pickle.dump(sig, f)

with open(data_path + 'mesa/10h_input/'+mode+'_fea.pkl', 'wb') as f:
    pickle.dump(fea, f)
    
with open(data_path + 'mesa/10h_input/'+mode+'_label.pkl', 'wb') as f:
    pickle.dump(label, f)'''
with open(data_path + 'mesa/10h_input/'+mode+'_label.pkl', 'wb') as f:
    pickle.dump(label, f)
