...
#for file_id in tqdm.tqdm(all_subjects):
    ...
    ppg_row = signal.resample(std_x, int(std_x.shape[0]/4))
    ...
    RR_list = f_hr([i for i in range(0, std_x.shape[0], 128)])/256#256
    blood_list = f_bl([i for i in range(0, std_x.shape[0], 128)])/256#256
    ba_list = signal.resample(signal.filtfilt(b_l, a_l, std_x), int(std_x.shape[0]/128))#256
    am_list = f_h([i for i in range(0, std_x.shape[0], 128)]) - f_l([i for i in range(0, std_x.shape[0], 128)])
    ...
    for i in tqdm.tqdm(range(0, int(std_x.shape[0]/128-40), 20)):
...
already = np.unique([i[-7:-3] for i in os.listdir("/extern2/zgz/wq/sleep/sleep_stage_ppg/clean_mesa_300s/"+mode+"/ppg/")])
