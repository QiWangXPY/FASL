...
from baseline_model_10h import PPG_10h, HeartRate_10h, HRV_10h, Hypnos_10h, Hypnos_10h_ablation, ConFusionLoss, FourSignal_10h, Feature_10h, ALL_10h, OurPPG_10h, Spectrum_PPG
...
mode = 'usleep'# all, ppg, sig, fea, hypnos, usleep
...
def main(args):
    ...
    for fold_num in np.arange(1):
        ...
        if mode == 'sig':
            ...
            model_teacher.softmax = nn.Softmax(dim = 2)
            #summary(model, (4, 300))  # print the model summary
        if mode == 'fea':
            model = HRV_10h()
            model_teacher = HRV_10h()
            #model = Feature_10h()
        ...
        if mode == 'usleep':
            ...
            model_teacher = Spectrum_PPG(**model_params)
        if mode == 'all':
            model = ALL_10h()
            model_teacher = ALL_10h()
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label) in enumerate(train_loader):
                ...
                if type(outputs) in (tuple, list):
                    if mode == 'hypnos':
                        ...
                        feature, outputs, lab = outputs[0], outputs[1], outputs[2]
                ...
                if(frag_label==1):
                    ...
                    if mode == 'ppg':
                        ppg_temp = ppg.to(device)
                        prob_tea = model_teacher(ppg_temp)[1][0, :y.shape[0], :].detach()
                    if mode == 'hypnos':
                        sig_temp = sig.to(device)
                        prob_tea = model_teacher(sig_temp)[1][0, :y.shape[0], :].detach()
                    if mode == 'usleep':
                        ppg_temp = ppg.to(device)
                        prob_tea = model_teacher(ppg_temp)[1][0, :y.shape[0], :].detach()
                    if mode == 'sig':
                    ...
                    if mode == 'fea':
                        fea_temp = fea[:, :, ::3].permute([0, 2, 1])
                        fea_temp = fea_temp.to(device)
                        prob_tea = model_teacher(fea_temp)[1][0, :y.shape[0], :].detach()
                    if mode == 'all':
                        prob_tea = model_teacher(ppg, sig, fea)[1][0, :y.shape[0], :].detach()
                    
                    entropy = - torch.sum(prob_tea * torch.log(prob_tea + 1e-10), dim=1)
                    n = len(prob_tea)
                    
                    low_k = int(n * 0.15)
                    low_k =  torch.kthvalue(entropy, low_k ).values
                    high_k = int(n * 0.85)
                    
                    #unsupervised loss
                    value, predicted = torch.max(prob_tea[:, :].data, dim=1)
                    train_loss = criterion( outputs[(y >= 0) & (entropy < low_k), :], predicted[(y>=0) & (entropy < low_k)] ) 

                    #contract loss
                    predicted = predicted[(y >= 0) & (entropy < low_k)]
                    positive = outputs[(y >= 0) & (entropy < low_k)]
                    
                    for i in range(len(predicted)):
                        posi_list[predicted[i]].append(positive[i])

                    for i in range(4):
    
                        posi_len = len(posi_list[i])
                        if(posi_len <= 1):
                            continue

                        prob_tea_i = prob_tea.clone()
                        prob_tea_i[:, i] = 0
                        entropy_i = - torch.sum(prob_tea_i * torch.log(prob_tea_i + 1e-10), dim=1)
                        high_k_i =  torch.kthvalue(entropy_i, high_k).values
                        negative = outputs[(y >= 0) & (entropy > high_k_i)]
                        
                        
                        lab_mean = torch.mean(torch.stack(posi_list[i]), axis=0)
                        
                        all_fea = torch.cat([torch.stack(posi_list[i]), negative], axis=0)
                        
                        seg_logits = torch.cosine_similarity(
                            lab_mean.unsqueeze(0), all_fea, dim=1
                        )
                        
                        label = torch.zeros(len(all_fea))
                        label[:posi_len] = 1
                        label = label.float().cuda()
                        
                        train_loss = train_loss + F.binary_cross_entropy_with_logits(
                            seg_logits, label
                        )
                if(frag_label==0):
                    if mode == 'hypnos':#+ frag_cri(outputs_osa, ahi_label) 
                        ...
                        train_loss = criterion( outputs[y>=0, :], y[y>=0] ) 
                #if(frag_label==0):
                ...
                if epoch >= 0:
                    with torch.no_grad():
                        ema_decay = min( 1 - 1 / ( (epoch - 0) * len(train_loader) + batch_idx + 1 ), 0.99 )
                        for t_params, s_params in zip(
                            model_teacher.parameters(), model.parameters()
                        ):
                            t_params.data = (
            ...
            with torch.no_grad():
                for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label) in enumerate(val_loader):
                    ...
                    if batch_idx < 5:
                        val_fc3.append(val_feature[y>=0])
        ...
        with torch.no_grad():
            for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label) in enumerate(test_loader):
                ...
                if batch_idx < 5:
                    test_fc3_feature.append(test_feature[y>=0])
