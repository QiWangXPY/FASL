...
from baseline_model_10h import PPG_10h, HeartRate_10h, HRV_10h, Hypnos_10h, Hypnos_10h_ablation, OurConLoss, ConFusionLoss, FourSignal_10h, Feature_10h, ALL_10h, OurPPG_10h, Spectrum_PPG
...
mode = 'all' # all, ppg, sig, fea, hypnos, usleep

dataset = 'selfcollect'#cfs
...
class Load_10h_Dataset(Dataset):
    def __init__(self, mode):
        ...
        if(self.mode == 'train'):
            ...
            with open(data_path + dataset +  '/overall/'+mode+'_ppg_44.pkl', 'rb') as f:
                self.ppg = pickle.load(f)
            #with open(data_path + dataset + '/overall/'+self.mode+'_unet_ppg_46.pkl', 'rb') as f:
                #self.ppg = pickle.load(f)
    ...
    def __getitem__(self, index):
        ...
        label = torch.tensor(self.test_label[index])[:1200]
        ...
        ahi_label = torch.tensor(self.ahi_label[index])[:1200]
...
class Load_10h_Dataset(Dataset):
    def __init__(self, mode):
        ...
        if(self.mode == 'train'):
            ...
            with open(data_path + dataset + '/device/10h_input/'+self.mode+'_fea.pkl', 'rb') as f:#200
                self.fea = pickle.load(f)
            #self.fea = torch.load(data_path + dataset + '/10h_input/'+self.mode+'_fea.pt').tolist()
            ...
            #with open(data_path + dataset + '/overall/'+self.mode+'_sig_conseq_40.pkl', 'rb') as f:#_500
            #    self.sig = pickle.load(f)
            with open(data_path + dataset + '/device/10h_input/'+self.mode+'_sig.pkl', 'rb') as f:#200
                self.sig = pickle.load(f)
            
            
            with open(data_path + dataset + '/device/10h_input/'+self.mode+'_ppg.pkl', 'rb') as f:#200
                self.ppg = pickle.load(f)
            #with open(data_path + dataset + '10h_input/'+self.mode+'_unet_ppg.pkl', 'rb') as f:#200
            #    self.ppg = pickle.load(f)
            ...
            with open(data_path + dataset + '/device/10h_input/'+self.mode+'_lab.pkl', 'rb') as f:#200
                self.test_label = pickle.load(f)
    ...
    def __getitem__(self, index):
        ...
        label = self.test_label[index][:1200] #1200
...
def main(args):
    ...
    class_counts = torch.tensor([726, 2555, 886, 1315])#label unbalence
    ...
    weight = torch.tensor([4, 10, 1.5, 2.6])#label unbalence
    ...
    for fold_num in np.arange(1):
        ...
        if mode == 'sig':
            ...
            #model = torch.load('tfboard/mesa/ResHybridAtt/Comparative_Experiment_mesa/heart_rate/hr_100_1/saved_models/fold_0_epoch_48.pth')
        ...
        if mode == 'all':
            ...
            #model = torch.load('tfboard/mesa/ResHybridAtt/overallperformance_cfs/sup_40/our_lstm_cfs_40/saved_models/fold_0_epoch_15.pth')
            #model = torch.load('tfboard/mesa/ResHybridAtt/overallperformance_cfs/sup_40/our_cnn_cfs_40/saved_models/fold_0_epoch_44.pth')
            ...
            #model = torch.load('tfboard/mesa/ResHybridAtt/Comparative_Experiment_abl/npppg/woppg_200_3/saved_models/fold_0_epoch_25.pth')
            #model = torch.load('tfboard/mesa/ResHybridAtt/Comparative_Experiment_abl/nosig/wosig_200_1/saved_models/fold_0_epoch_31.pth')
            #model = torch.load('tfboard/mesa/ResHybridAtt/Comparative_Experiment_abl/nofea/wofea_200_1/saved_models/fold_0_epoch_24.pth')
            #model = torch.load('tfboard/mesa/ResHybridAtt/Comparative_Experiment_abl/lstm/lstm_200_9/saved_models/fold_0_epoch_27.pth')

            #model = torch.load('tfboard/mesa/ResHybridAtt/Comparative_Experiment_abl/attn_200_mesa/saved_models/fold_0_epoch_51.pth')
        
            #model = torch.load('tfboard/mesa/ResHybridAtt/our_linear_test/saved_models/fold_0_epoch_29.pth')
            #model = torch.load('tfboard/mesa/ResHybridAtt/enc/enc_345_2/saved_models/fold_0_epoch_33.pth')
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label) in enumerate(train_loader):
                ...
                if(frag_label==1):
                    ...
                    output_temp = outputs[entropy_i < low_k]
                    
                    prob_tea_i = outputs[entropy_i < low_k].clone()
                    prob_fea = feature[entropy_i < low_k, :]#feature, :y.shape[0]
                    ...
                    for i in range(4):
                        for j in range(len(prob_tea_i)):
                            posi_list[i].append( prob_tea_i[j][i]-entropy_i[j]  )#############权重 * weight[i]
                    posi_list = torch.clamp(torch.stack([torch.stack(i) for i in posi_list]), min=0).detach()
    
                    con_fea = torch.zeros([4, torch.sum(posi_list>0, axis = 1).min(), prob_fea.shape[1]])

                    n = len(entropy_i)
                    
                    low_k = int(n * 0.15)
                    low_k =  torch.kthvalue(entropy_i, low_k ).values
                    value, predicted = torch.max(output_temp[:, :].data, dim=1)
                    
                    train_loss = criterion( output_temp[entropy_i < low_k, :], predicted[entropy_i < low_k] )
                    #train_loss = 0
                    ...
                    if(con_fea.shape[1] != 0):
                        ...
                        for i in range(4):    
                            ...
                            temp = (posi_list[i].unsqueeze(1) * prob_fea + (1 - posi_list[i]).unsqueeze(1) * lab_mean)[posi_list[i]>0]#(prob_fea)[posi_list[i]>0] #(prob_fea)[posi_list[i]>0] #
                            indices = torch.randperm(len(temp))[:con_fea.shape[1]]
                            con_fea[i] = temp[indices]
                    ...
                    #train_loss = 0
                    train_loss = criterion( outputs[(y>=0) & (entropy_i < low_k), :], predicted[(y>=0) & (entropy_i < low_k)] )
                    ...
                    if(con_fea.shape[1] != 0):
                        ...
                        train_loss = train_loss + con_criterion(con_fea)
                    
                if(frag_label==0):
                    if mode == 'hypnos': # frag_cri(outputs_osa, ahi_label) + 
                        train_loss = criterion(outputs[y>=0], y[y>=0]) + con_criterion(feature[:, :y.shape[0], :].permute(1, 0, 2))
                        feature = feature[0, :y.shape[0], :]
                    else:
