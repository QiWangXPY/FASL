...
def main(args):
    ...
    for fold_num in np.arange(1):
        ...
        if mode == 'sig':
            ...
            #model = torch.load('tfboard/mesa/ResHybridAtt/overallperformance_cfs/sup_40/hr_cfs_40/saved_models/fold_0_epoch_34.pth')
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label) in enumerate(train_loader):
                ...
                if(frag_label==1): #and epoch>=2):
                    ...
                    if mode == 'usleep':
                        ppg_50 = ppg.cpu().permute([0, 1, 3, 2])
                        ppg_50[0] = mask_recover(ppg_50[0], 0.5)
                        ppg_50 = ppg_50.permute([0, 1, 3, 2]).cuda()
                    else:
                        ppg_50 = mask_recover(ppg.cpu(), 0.5).cuda()
                    sig_50 = mask_recover(sig.cpu(), 0.5).cuda()
                    fea_50 = mask_recover(fea.cpu(), 0.5).cuda()
                    
                    if mode == 'ppg':
                        ppg_50 = ppg_50.to(device)
                        prob_50 = model(ppg_50)[1][0, :y.shape[0], :]
                    if mode == 'sig':
                        sig_temp = sig_50[0, :1]
                        sig_temp = sig_temp.to(device)
                        prob_50 = model(sig_temp)[1][0, :y.shape[0], :]
                    if mode == 'fea':
                        fea_50 = fea_50[:, :, ::3].permute([0, 2, 1])
                        fea_50 = fea_50.to(device)
                        prob_50 = model(fea_50)[1][0, :y.shape[0], :]
                    if mode == 'usleep':
                        ppg_50 = ppg_50.to(device)
                        prob_50 = model(ppg_50)[1][0, :y.shape[0], :]
                    if mode == 'hypnos':
                        ...
                        #ppg = ppg.to(device)
                        #outputs = model(ppg)
                        sig_temp = sig_50.to(device)
                        prob_50 = model(sig_temp)[1][0, :y.shape[0], :]
                    if mode == 'all':
                        prob_50 = model(ppg_50, sig_50, fea_50)[1][0, :y.shape[0], :]
                    ...
                    train_loss = criterion( prob_50[(y>=0) & (entropy < low_k)], predicted[(y>=0) & (entropy < low_k)] ) 
                ...
                if(frag_label==0):
                    if mode == 'hypnos':
                        ...
                        train_loss = criterion(outputs[y>=0, :], y[y>=0]) + con_criterion(feature[:, y>=0, :].permute(1, 0, 2))#frag_cri(outputs_osa, ahi_label) 
                    else:
