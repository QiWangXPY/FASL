...
class Loadone_Dataset(Dataset):
    ...
    def __init__(self, mode):
        ...
        with open(data_path + 'mesa/'+mode+'_exc_label.pkl', 'rb') as f:
            exc_label = pickle.load(f)
        with open(data_path + 'mesa/'+mode+'_frag_label.pkl', 'rb') as f:
            frag_label = pickle.load(f)
        ...
        self.len = 4 * length#label.shape[0]
...
def main(args):
    ...
    for fold_num in np.arange(1):
        ...
        for epoch in range(args.epochs):
            ...
            model.train()
            for batch_idx, (ppg, sig, fea, label) in enumerate(train_loader):
            ...
            model.eval()
            with torch.no_grad():
                for batch_idx, (ppg, sig, fea, label) in enumerate(val_loader):
        ...
        model.eval()
        with torch.no_grad():
            for batch_idx, (ppg, sig, fea, label) in enumerate(test_loader):
