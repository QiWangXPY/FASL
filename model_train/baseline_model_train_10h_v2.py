...
class Load_10h_Dataset(Dataset):
    def __init__(self, mode):
        ...
        if(self.mode == 'train'):
            ...
            with open(data_path + 'mesa/10h_input/'+self.mode+'_sig_conseq.pkl', 'rb') as f:#_500
                self.sig = pickle.load(f)
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label) in enumerate(train_loader):
                ...
                if batch_idx > 0 and args.debug == 1:
                    continue
                ppg_next, sig_next, fea_next, label_next, frag_label_next, ahi_label_next = next(iter(train_loader)) 

                slice = np.random.randint(0, min(label.shape[1], label_next.shape[1]), [1])[0]
                
                ppg = torch.concat([ppg[:,:,:slice*1024], ppg_next[:,:,slice*1024:]], axis = 2)
                sig = torch.concat([sig[:,:,:slice*60], sig_next[:,:,slice*60:]], axis = 2)
                fea = torch.concat([fea[:,:,:slice*3-2], fea_next[:,:,slice*3-2:]], axis = 2)
                label = torch.concat([label[:, :slice], label_next[:,slice:]], axis = 1)
