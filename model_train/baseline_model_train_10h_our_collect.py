...
from baseline_model_10h import PPG_10h, HeartRate_10h, HRV_10h, Hypnos_10h, Hypnos_10h_ablation, ConFusionLoss, FourSignal_10h,\
            Feature_10h, ALL_10h, OurPPG_10h, Spectrum_PPG, ALL_10h_30
...
class Load_10h_Dataset(Dataset):
    def __init__(self, mode):
        ...
        if(self.mode == 'train'):
            with open(data_path + 'mesa/10h_input/'+self.mode+'_fea_conseq_1654.pkl', 'rb') as f:
                self.fea = pickle.load(f)
            #with open(data_path + 'mesa/10h_input/'+mode+'_sig.pkl', 'rb') as f:
            #    self.sig = pickle.load(f)
            with open(data_path + 'mesa/10h_input/'+self.mode+'_sig_conseq_1654.pkl', 'rb') as f:#_500
                self.sig = pickle.load(f)
            
            with open(data_path + 'mesa/10h_input/'+mode+'_ppg_1654.pkl', 'rb') as f:
                self.ppg = pickle.load(f)
            #with open(data_path + 'mesa/unet_sleep/'+mode+'_ppg_200.pkl', 'rb') as f:
                #self.ppg = pickle.load(f)
                
            with open(data_path + 'mesa/10h_input/'+mode+'_label_frag_1654.pkl', 'rb') as f:
                self.test_label = pickle.load(f)
            with open(data_path + 'mesa/10h_input/'+self.mode+'_ahi_label_1654.pkl', 'rb') as f:
                self.ahi_label = pickle.load(f)
        else:
...
class Load_10h_Dataset(Dataset):
    def __init__(self, mode):
        ...
        if(self.mode == 'train'):
            ...
            #    self.fea = pickle.load(f)
            with open(data_path + 'selfcollect/10h_input/'+self.mode+'_fea.pkl', 'rb') as f:
                self.fea = pickle.load(f)
            with open(data_path + 'selfcollect/10h_input/'+self.mode+'_sig.pkl', 'rb') as f:
                self.sig = pickle.load(f)
            with open(data_path + 'selfcollect/10h_input/'+self.mode+'_ppg.pkl', 'rb') as f:
                self.ppg = pickle.load(f)

            with open(data_path + 'selfcollect/10h_input/'+self.mode+'_lab.pkl', 'rb') as f:
                self.test_label = pickle.load(f)
            #with open(data_path + dataset + '/10h_input/'+mode+'_label_frag_200.pkl', 'rb') as f:
            #    self.test_label = pickle.load(f)
                            
            #with open(data_path + dataset + '/10h_input/'+self.mode+'_ahi_label_200.pkl', 'rb') as f:
            #    self.ahi_label = pickle.load(f)
        else:
            ...
            with open(data_path + 'selfcollect/device/10h_input/'+self.mode+'_fea.pkl', 'rb') as f:
                self.fea = pickle.load(f)
            with open(data_path + 'selfcollect/device/10h_input/'+self.mode+'_sig.pkl', 'rb') as f:
                self.sig = pickle.load(f)
            with open(data_path + 'selfcollect/device/10h_input/'+self.mode+'_ppg.pkl', 'rb') as f:
                self.ppg = pickle.load(f)

            with open(data_path + 'selfcollect/device/10h_input/'+self.mode+'_lab.pkl', 'rb') as f:
                self.test_label = pickle.load(f)
    ...
    def __getitem__(self, index):
        ...
        return ppg.float(), sig.float(), fea.float(), label.long(), frag_label.long()
...
def main(args):
    ...
    class_counts = torch.tensor([209643,  25493])#torch.tensor([209643,  25493])#torch.tensor([14, 4])
    total_samples = class_counts.sum()
    weights = total_samples / class_counts  # 计算权重
    weights = weights / weights.sum() 
    ...
    for fold_num in np.arange(1):
        ...
        if mode == 'all':
            ...
            model = torch.load('tfboard/mesa/ResHybridAtt/overall_self/our+our/saved_models/fold_0_epoch_25.pth')
            
            #model = torch.load('tfboard/mesa/ResHybridAtt_OLD/our_10h_all_cnn/saved_models/fold_0_epoch_4.pth')
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label) in enumerate(val_loader):
                ...
                optimizer.step()
    		'''
            for batch_idx, (ppg, sig, fea, label, frag_label) in enumerate(train_loader):
                ...
                if mode == 'hypnos':
                    ...
                    #ppg = ppg.to(device)
                    #outputs = model(ppg)
                    sig = torch.concat([sig, sig], axis = 0)
                    sig = sig.to(device)
                    outputs = model(sig)
                if mode == 'usleep':
                    ppg = ppg.to(device)
                    outputs = model(ppg)
                if mode == 'all':
                    #ppg = ppg.reshape([1, 1200, 1024]).permute([1, 0, 2])
                    #sig = sig.reshape([4, 1200, 60]).permute([1, 0, 2])
                    #fea = fea[:, :, ::3].permute([2, 1, 0])
                ...
                #if mode == 'ppg':
                #    train_loss = frag_cri( outputs[:, :], ahi_label ) #+ train_loss + ahi_cri( ahi_outputs[:, :2], y_ahi )
                if mode == 'hypnos':
                    train_loss = criterion(outputs, y) + con_criterion(feature[:, :y.shape[0], :].permute(1, 0, 2))
                else:
                ...
                #_, predicted = torch.max(outputs_osa[:, :].data, dim=1)
            ...
            with torch.no_grad():
                for batch_idx, (ppg, sig, fea, label, frag_label) in enumerate(val_loader):
                    ...
                    #val_feature = val_feature[:y.shape[0], 0, :]
                    #y_val_pred = y_val_pred[:y.shape[0], 0, :]
                    
                    #y_val_pred = y_frag_prob[0, :y.shape[0], :]
        ...
        #classifier.eval()
        with torch.no_grad():
            for batch_idx, (ppg, sig, fea, label, frag_label) in enumerate(test_loader):
                ...
                #test_feature = test_feature[:y.shape[0], 0, :]
                #y_test_prob = y_test_prob[:y.shape[0], 0, :]
                
                #y_test_prob = y_frag_prob[0, :y.shape[0], :]
