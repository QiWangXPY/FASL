    ...
    def __init__(
        ...
        for i_block in range(n_blocks):
            ...
            out_channels += len(kernels) * \
                len(layer_dilations)*channel_per_kernel#1 * 32    #1
...
class DenseNet(nn.Module):
    def __init__(
        ...
        #self.classifier_half = nn.Linear(112, n_classes)

        #self.conclassifier = nn.Linear(3 * out_channels, n_classes)
    ...
    def forward(
        ...
        self.debug(f'input: {x.shape}')
        if x.shape[1] == 1:
            x = self.low_conv(x)#32,192
        out = x
        self.debug(f'low conv: {out.shape}')

        for i_block in range(len(self.dense_blocks)):
            out = self.dense_blocks[i_block](out)#96,192
            self.debug(f'dense-{i_block}: {out.shape}')
            if self.skip_final_transition_blk \
                    and i_block == len(self.dense_blocks) - 1:#1:
                break

            '''
            if i_block == len(self.dense_blocks)-2:
                out_half = self.gap(out[:, :, 48:144])
                out_half = self.dropout(out_half)
                out_half = out_half.view(out_half.size(0), -1)
                out_half = self.classifier_half(out_half)
            '''
        ...
        #out = torch.concat([out.unsqueeze(1), out_half.unsqueeze(1)], axis = 1)
        ...
        for x in self.low_conv_cfg:
            if x == 'M':
                ...
                layers += [nn.MaxPool1d(kernel_size=pooling_kr,
                                        stride=pooling_kr)]#384-192
                i_pooling += 1
            else:
                conv_zero_padd = int(
                    (self.low_conv_kernel[i_conv_layer] - 1) // 2)#21
                layers += [
                    nn.Conv1d(
                        in_channels, x,
                        kernel_size=self.low_conv_kernel[i_conv_layer],#[21, 21],
                        padding=conv_zero_padd,
                        stride=self.low_conv_strides[i_conv_layer]),#[5, 1],
                    nn.BatchNorm1d(x),
                    nn.ReLU(inplace=True)
                ]#1920-384-384
                in_channels = x
...
class PPGNet(nn.Module):
    def __init__(self):
        ...
        self.conclassifier = nn.Linear(232, NUM_CLASSES)
    ...
    def forward(
        ...
        out_1 = self.model_300(x)
        out_2 = self.model_150(x[:, :, 75*64:225*64])
        fea = torch.concat([out_1[0],out_2[0]], axis = 1)
        out = self.conclassifier(fea)
        out = torch.concat([out.unsqueeze(1), out_1[1].unsqueeze(1), out_2[1].unsqueeze(1)], axis = 1)
        return fea, out
...
if __name__ == '__main__':
    ...
    x = torch.randn(1, IN_CHAN, Hz*SEG_SEC)
    out = model(x)
