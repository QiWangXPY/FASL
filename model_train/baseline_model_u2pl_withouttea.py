        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label) in enumerate(train_loader):
                ...
                if(frag_label==1):
                    ...
                    #unsupervised loss
                    value, predicted = torch.max(outputs[:, :].data, dim=1)
                    train_loss = criterion( outputs[entropy < low_k, :], predicted[entropy < low_k] ) 

                    #contract loss
                    predicted = predicted[entropy < low_k]
                    positive = outputs[entropy < low_k]
                    ...
                    for i in range(4):
                        ...
                        negative = outputs[entropy > high_k_i]
                        ...
                        train_loss = train_loss + F.cross_entropy(
                            seg_logits / 0.5, label
                        )/(posi_len + high_k)
