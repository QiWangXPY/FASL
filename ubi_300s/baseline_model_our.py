...
from baseline_model_10h import PPG_10h, HeartRate_10h, HRV_10h, Hypnos_10h, Hypnos_10h_ablation, ConFusionLoss, FourSignal_10h, Feature_10h, ALL_10h, OurPPG_10h
                ...
                if(frag_label==1):
                    ...
                    for i in range(4):
                        for j in range(len(outputs)):
                            posi_list[i].append(outputs[j][i]-entropy_i[j])
                    posi_list = torch.clamp(torch.stack([torch.stack(i) for i in posi_list]), min=0)
                    con_fea = torch.zeros([4, torch.sum(posi_list>0, axis = 1).min(), 4])
                    ...
                    if(con_fea.shape[1] != 0):
                        #contract loss
                        for i in range(4):    
                            lab_mean = torch.sum(posi_list[i].unsqueeze(1) * outputs, axis = 0)/torch.sum(posi_list[i])
                            
                            temp = (posi_list[i].unsqueeze(1) * outputs + (1 - posi_list[i]).unsqueeze(1) * lab_mean)[posi_list[i]>0]
