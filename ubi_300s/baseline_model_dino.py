        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label) in enumerate(train_loader):
                ...
                if(frag_label==1): #and epoch>=2):
                    ...
                    if mode == 'ppg':
                        ppg_temp = ppg.to(device)
                        prob_tea = model_teacher(ppg_temp)[1][0, :y.shape[0], :]
                    if mode == 'hypnos':
                    ...
                    train_loss = criterion( outputs[(y>=0) & (entropy < low_k)], prob_tea[(y>=0) & (entropy < low_k)] ) 
                ...
                if epoch >= 0:
                    with torch.no_grad():
                        ema_decay = min( 1 - 1 / ( (epoch - 0) * len(train_loader) + batch_idx + 1 ), 0.99 )#-2
                        for t_params, s_params in zip(
                            model_teacher.parameters(), model.parameters()
                        ):
                            t_params.data = (
