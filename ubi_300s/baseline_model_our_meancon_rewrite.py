...
class Load_10h_Dataset(Dataset):
    def __init__(self, mode):
        ...
        if(self.mode == 'train'):
            with open(data_path + 'mesa/10h_input/'+self.mode+'_fea_conseq_200.pkl', 'rb') as f:#500
                self.fea = pickle.load(f)
            #with open(data_path + 'mesa/10h_input/'+mode+'_sig.pkl', 'rb') as f:
            #    self.sig = pickle.load(f)
            with open(data_path + 'mesa/10h_input/'+self.mode+'_sig_conseq_200.pkl', 'rb') as f:
                self.sig = pickle.load(f)
...
def main(args):
    ...
    for fold_num in np.arange(1):
        ...
        if mode == 'all':
            ...
            model = torch.load('tfboard/mesa/ResHybridAtt/our_10h_all_cnn_200/saved_models/fold_0_epoch_46.pth')
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label) in enumerate(train_loader):
                ...
                if(frag_label==1):
                    ...
                    con_fea = torch.zeros([1, 4, torch.sum(posi_list>0, axis = 1).min(), prob_fea.shape[1]])
                    con_mean = torch.zeros([4, 1, 1, prob_fea.shape[1]])
                    ...
                    if(con_fea.shape[2] != 0):
                        ...
                        for i in range(4):
                            ...
                            indices = torch.randperm(len(temp))[:con_fea.shape[2]]
                            con_fea[0, i] = temp[indices]
                            con_mean[i, 0, 0] = lab_mean
                        
                        temp_sim = torch.exp(torch.cosine_similarity(con_mean, con_fea, dim = -1))
                        for i in range(con_mean.shape[0]):
                            temp_pos = temp_pos + torch.log(torch.sum(temp_sim[i, i]))
                            mask = torch.ones(temp_sim[i].shape, dtype=torch.bool)
                            mask[i, :] = False 
                            temp_neg = temp_neg + torch.log(torch.sum(temp_sim[i][mask]))
                    train_loss = train_loss + temp_neg/4 - temp_pos/4
                    '''
                    n = len(outputs)
                    low_k = int(n * 0.5)
                    entropy_i = - torch.sum(outputs * torch.log(outputs + 1e-10), dim=1)
