...
class Load_10h_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        frag_label = torch.tensor(0)
        if(self.mode == 'train' and index >= self.train_len):  #无标注
            frag_label = torch.tensor(1)
            
            ind = torch.randperm(self.train_len)[0]
            ppg_la = self.ppg[ind].unsqueeze(0)
            sig_la = self.sig[ind]
            fea_la = self.fea[ind][self.mask]

            
            label_center = self.test_label[ind][0, :1200] #1200

            ppg = torch.concat([ppg_la.unsqueeze(0), ppg.unsqueeze(0)], axis = 0)
            sig = torch.concat([sig_la.unsqueeze(0), sig.unsqueeze(0)], axis = 0)
            fea = torch.concat([fea_la.unsqueeze(0), fea.unsqueeze(0)], axis = 0)
        
        ahi_label = self.ahi_label[index][:1200] #1200
        
        return ppg.float(), sig.float(), fea.float(), label.long(), frag_label.long(), ahi_label.long(), label_center.long()
...
def main(args):
    ...
    for fold_num in np.arange(1):
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label, label_center) in enumerate(train_loader):
                ...
                label_center = label_center.squeeze(0).to(device)
                y = y.long().to(device)
                ...
                if mode == 'all':
                    ...
                    '''
                    if(frag_label==1):
                        output_center = model(ppg[:1], sig[:1], fea[:1])
                        outputs = model(ppg[1:], sig[1:], fea[1:])
                    else:
                ...
                if(frag_label==1):
                    feature_center, output_center = output_center[0], output_center[1]
                    feature_center = feature_center[0, :label_center.shape[0], :]
                    output_center = output_center[0, :label_center.shape[0], :]
                ...
                if(frag_label==1):
                    ...
                    if(con_fea.shape[1] != 0):
                        ...
                        for i in range(4):    
                            ...
                            label_weight = torch.tensor([int(j) for j in label_center==i]).to(device)

                            lab_mean = torch.sum(label_weight.unsqueeze(1) * feature_center, axis = 0)/torch.sum(label_weight)
                            
                            temp = (posi_list[i].unsqueeze(1) * prob_fea + (1 - posi_list[i]).unsqueeze(1) * lab_mean[i].unsqueeze(0))
                            
                            temp = temp[posi_list[i]>0]
            ...
            with torch.no_grad():
                for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label, label_center) in enumerate(val_loader):
        ...
        #classifier.eval()
        with torch.no_grad():
            for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label, label_center) in enumerate(test_loader):
