...
def main(args):
    ...
    for fold_num in np.arange(1):
        ...
        if mode == 'all':
            model = ALL_10h()
            model = torch.load('tfboard/mesa/ResHybridAtt/our_10h_all_cnn_500_twoclasslayer/saved_models/fold_0_epoch_33.pth')
            model.relu = nn.ReLU()
            model.batch_norm1 = nn.BatchNorm1d(64)
            model.batch_norm2 = nn.BatchNorm1d(4)
            ...
            for batch_idx, (ppg, sig, fea, label, frag_label, ahi_label) in enumerate(train_loader):
                ...
                if(frag_label==1):
                    ...
                    if(con_fea.shape[1] != 0):
                        #contract loss
                        for i in range(4):    
                            lab_mean = torch.sum(posi_list[i].unsqueeze(1) * feature, axis = 0)/torch.sum(posi_list[i])
                        ...
                        train_loss = con_criterion(con_fea)
